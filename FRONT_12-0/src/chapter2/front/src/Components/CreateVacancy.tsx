...
export default function CreateVacancy() {
    ...
    const lvl = useRef<HTMLSelectElement>(null);
    const grade = useRef<HTMLSelectElement>(null);
    ...
    const create_vacancy = React.useCallback(() => {
        fetch("http://localhost:3000/create-vacancy", {
            ...
            body: JSON.stringify({
                title: title.current? title.current.value: undefined,
                description: description.current? description.current.value: undefined,
                english: lvl.current? lvl.current.value: undefined,
                grade: grade.current? grade.current.value: undefined})
