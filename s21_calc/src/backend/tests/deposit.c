...
START_TEST(deposit_1) {
    int error = 0;
    t_deposit res = deposit_calc(1000, 12, 12, 0, 1, 0, "1 1000", "1 1000", &error);
...
START_TEST(deposit_2) {
    int error = 0;
    t_deposit res = deposit_calc(1000, 12, 12, 0, 1, 1, "1 1000", "1 1000", &error);

    ck_assert_int_eq(error, 0);
    ck_assert_double_eq_tol(res.tax, 0, 1e-7);
    ck_assert_double_eq_tol(res.contribution, 1126.83, 1e-7);
}
END_TEST
