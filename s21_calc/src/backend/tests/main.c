...
int main() {
  Suite *list_cases[] = {
      create_simple_int_testcase(),   create_simple_float_testcase(),
      create_trigonometry_testcase(), create_bad_testcase(),
      create_hard_testcase(),         create_credit(),
      create_deposit_testcase(),      NULL};
...
long double get_EPS(long double true_result, long double result) {
  if (fabsl(result) > fabsl(true_result)) result = true_result;
