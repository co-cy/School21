...
MainWindow::MainWindow(QWidget *parent)
  ...
  ui->setupUi(this);
  converter = {
      {ui->asin->objectName(), "asin ("},
      {ui->sin->objectName(), "sin ("},
      {ui->acos->objectName(), "acos ("},
      {ui->cos->objectName(), "cos ("},
      {ui->tan->objectName(), "tan ("},
      {ui->atan->objectName(), "atan ("},
      {ui->sqrt->objectName(), "sqrt ("},
      {ui->pow->objectName(), "^"},
      {ui->log->objectName(), "log ("},
      {ui->nat_log->objectName(), "ln ("},
      {ui->number_0->objectName(), "0"},
      {ui->number_1->objectName(), "1"},
      {ui->number_2->objectName(), "2"},
      {ui->number_3->objectName(), "3"},
      {ui->number_4->objectName(), "4"},
      {ui->number_5->objectName(), "5"},
      {ui->number_6->objectName(), "6"},
      {ui->number_7->objectName(), "7"},
      {ui->number_8->objectName(), "8"},
      {ui->number_9->objectName(), "9"},
      {ui->number_x->objectName(), "x"},
      {ui->open_bracket->objectName(), "("},
      {ui->close_bracket->objectName(), ")"},
      {ui->sum->objectName(), "+"},
      {ui->sub->objectName(), "-"},
      {ui->mul->objectName(), "*"},
      {ui->div->objectName(), "/"},
      {ui->mod->objectName(), "mod"},
      {ui->point->objectName(), "."},
  ...
  connect(ui->calc_deposit, SIGNAL(clicked()), this, SLOT(calc_deposit()));
  ...
  ui->customPlot->setBackground(
      ui->graph_grid->palette().color(ui->graph_grid->backgroundRole()));

  ui->customPlot->xAxis->setBasePen(QPen(Qt ::white, 1));  // Перо оси
  ui->customPlot->xAxis->setTickPen(QPen(Qt ::white, 1));  // Перо отметки оси
  ui->customPlot->xAxis->setSubTickPen(
      QPen(Qt ::white, 1));  // Перо отметки оси
  ui->customPlot->xAxis->setTickLabelColor(
  ...
  ui->customPlot->yAxis->setSubTickPen(
      QPen(Qt ::white, 1));  // Перо отметки оси
  ui->customPlot->yAxis->setTickLabelColor(
      Qt ::white);  // цвет текста шкалы оси
...
void MainWindow::calc_credit() {
  if (this->ui->annuity->isChecked()) {
    ...
    double start_res =
        credit_calc_differentiate(loan, period, 1, this->ui->percent->value());
    double end_res = 0;
    if (period > 1)
...
void MainWindow::calc_deposit() {
  ...
  t_deposit a = deposit_calc(deposit_amount, this->ui->deposit_period->value(),
                             this->ui->deposit_percent->value(),
                             this->ui->deposit_tax_percent->value(), pay_period,
                             this->ui->capitalize->isChecked(), list_add,
                             list_remove, &error_code);
  if (error_code) {
    ...
    this->ui->label_sum_tax->setText("Сумма налога: " + QString::number(a.tax));
