...
class S21Matrix {
 ...
 public:
  ...
  void SumMatrix(const S21Matrix &other) { MergeMatrix(other, 1); }
  void SubMatrix(const S21Matrix &other) { MergeMatrix(other, -1); }
  ...
  [[nodiscard]] bool IsEqSize(const S21Matrix &other) const noexcept {
  ...
  void SetRows(int rows) { Resize(rows, GetCols(), this); }
  void SetCols(int cols) { Resize(GetRows(), cols, this); };
...
S21Matrix operator*(double num, const S21Matrix &a) noexcept;
S21Matrix operator*(const S21Matrix &a, const S21Matrix &b);
